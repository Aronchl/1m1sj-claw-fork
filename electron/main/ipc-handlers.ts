 ...
 *
 * Physically deletes the session transcript `*.jsonl` (and legacy `*.deleted.jsonl` if present),
 * then removes the entry from `sessions.json`.
...
function registerSessionHandlers(): void {
  ipcMain.handle('session:delete', async (_, sessionKey: string) => {
    try {
      logger.info(`[session:delete] key=${sessionKey}`);
      const result = await executeDeleteSessionTranscript(sessionKey);
      if (!result.ok) {
        logger.warn(`[session:delete] Failed: ${result.error}`);
        return { success: false, error: result.error };
      }
      logger.info(`[session:delete] OK ${sessionKey}`);
      return { success: true };
    } catch (err) {
