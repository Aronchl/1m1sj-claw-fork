...
const OPENCLAW_LOG_DIR = join(tmpdir(), 'openclaw');
const OPENCLAW_LOG_MAX_BYTES = 450 * 1024 * 1024;
...
function cleanupOversizedOpenClawLogs(): void {
  try {
    if (!existsSync(OPENCLAW_LOG_DIR)) return;
    const names = readdirSync(OPENCLAW_LOG_DIR);
    for (const name of names) {
      if (!/^openclaw-\d{4}-\d{2}-\d{2}\.log$/.test(name)) continue;
      const filePath = join(OPENCLAW_LOG_DIR, name);
      let size = 0;
      try {
      ...
      if (size < OPENCLAW_LOG_MAX_BYTES) continue;
      try {
        rmSync(filePath, { force: true });
        logger.info(`[gateway-log] Removed oversized OpenClaw log: ${filePath} (${size} bytes)`);
      } catch (err) {
        logger.warn(`[gateway-log] Failed to remove oversized OpenClaw log ${filePath}:`, err);
      }
    }
  } catch (err) {
    logger.warn('Failed to inspect OpenClaw temp log directory:', err);
  }
}
