...
On each launch, ClawX **replaces** the entire `~/.openclaw/skills` tree with only the skills listed in the bundled `resources/skills/preinstalled-manifest.json` (skills installed via ClawHub or copied manually into that folder are **not** preserved). The `skills.entries` object in `~/.openclaw/openclaw.json` is pruned to those manifest slugs (keeping existing per-skill settings only for keys that remain).  
