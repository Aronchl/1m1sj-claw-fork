...
if (fs.existsSync(OUTPUT)) {
  // Transient ENOTEMPTY can happen if the directory is being touched by
  // background indexers/watchers; retry a few times before failing.
  let removed = false;
  let lastErr = null;
  for (let i = 0; i < 5; i++) {
    try {
      fs.rmSync(OUTPUT, { recursive: true, force: true, maxRetries: 3, retryDelay: 120 });
      removed = true;
      lastErr = null;
      break;
    } catch (e) {
